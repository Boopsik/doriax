...
std::string AiService::buildSystemPrompt() const {
    ...
    prompt
        ...
        << "Custom shaders are per-component on Mesh/UI/Points/Lines/Sky via the customShader property. To customize, call fork_shader (it copies the built-in <type>.vert/.frag into a project-relative directory and sets customShader in one undoable step; directory defaults to shaders, and fork_includes optionally copies the engine includes into an editable includes/ folder beside the fork). Never set customShader by hand to files that do not exist; the value is a base path with no extension (vert=base.vert, frag=base.frag) or two explicit paths joined by '|' (\"a.vert|b.frag\") when the entry points have different names. Both a .vert and a .frag are always required. Clear customShader (empty string) to reset to the built-in shader. A Mesh fork drives the color pass only: when it displaces vertices or discards, also fork the depth shader (fork_shader with shader_type depth sets customDepthShader) so shadow maps follow - and SSAO/post-process depth while SSR is off (with SSR on they read the G-buffer, which keeps the built-in shader); it reads the same shaderUniforms.\n"
        << "The forked .vert/.frag are complete, compiling engine shaders with a strict structure: a #version line, #include \"includes/...\" (resolved against the engine library), #ifdef variant guards, uniform blocks, texture and sampler declarations, in/out varyings, and a specific fragment output variable. Editing them with write_shader_file REQUIRES first calling read_resource_file on the forked file, then making minimal targeted changes that PRESERVE the #version, #include lines, uniform blocks, ALL original texture and sampler declarations (uniform sampler2D/samplerCube/textureN/samplerN and their #ifdef guards), in/out declarations, and the existing output variable name. Keep every original texture/sampler declaration even if your new code does not sample it - the engine binds them by fixed slot, so removing or renaming one breaks the texture bindings. Likewise keep the original main() code that READS the in varyings (v_position, v_normal, v_uv1, v_color, v_lightProjPos, etc.): if you delete a varying's usage the GLSL compiler strips that fragment input and the shader fails to cross-compile with a vertex/fragment interface mismatch (\"vertex shader output X does not exist in fragment shader inputs\"). Prefer to keep the original main() body that consumes the varyings and layer your effect on the final color (e.g. compute the original color, then posterize/override it), rather than deleting the existing computations. Only change the shading math (usually near the end of main()). Never rewrite a shader from scratch, drop the #version/#include lines, or invent sampler, texture, output or engine uniform names - that produces compile errors (e.g. missing precision qualifier / wrong #version) or broken bindings. A stylized custom shader (e.g. toon/cel) may legitimately replace the lighting model and stop sampling the shadow or IBL maps; that is allowed - keep their declarations (the engine harmlessly skips the now-unused bindings), but briefly tell the user when your shader changes lighting behavior such as no longer receiving shadows, so they can ask to keep it. If a custom shader fails to compile, the object falls back to the built-in shader and the error appears in the output window; read that error and the forked source, then fix minimally.\n"
        << "The one uniform block a fork MAY add is its own custom block: u_vs_customParams in the .vert and/or u_fs_customParams in the .frag (a post-process pass uses u_fs_postParams), e.g. uniform u_fs_customParams { float time; float amplitude; vec4 tint; } customParams; - keep the existing engine blocks untouched and put your effect parameters in this block with member names of your choice. Members are filled BY NAME from the component's shaderUniforms (or the pass uniforms): set them with set_shader_uniform (undoable, no shader rebuild) and list the declared members with inspect_shader_uniforms; scripts use mesh:setShaderUniform(\"amplitude\", 0.3) / scene:setPostProcessUniform(0, \"amount\", 0.5). The engine writes the members named time (seconds since startup) and resolution (xy = render target size, zw = 1/size) every frame, so an animated effect (waves, fire, scrolling) just declares float time and reads it - never set those two. Keep one scalar kind per block (do not mix int and float members: GL flattens the block after its first member) and avoid matrix and array members (they take no value and stay zero; declare one scalar/vector member per value). An unset member reads zero, so set a value or give the effect a sensible fallback.\n";
    return prompt.str();
}
